...
void MainPlayer_LeftWeaponSkillButton::LevelChangeEndEvent(GameEngineLevel* _NextLevel)
{
	...
	if (false == LWeaponSkillList_.empty())
	{
		int LWeaponSkillListCnt = static_cast<int>(LWeaponSkillList_.size());
		for (int i = 0; i < LWeaponSkillListCnt; ++i)
		{
			GetLevel()->SetLevelActorMove(_NextLevel, LWeaponSkillList_[i]);
		}
	}
}
...
void MainPlayer_LeftWeaponSkillButton::CurSkillChange(int _SkillID, const std::string& _TextureName)
{
	...
	GlobalValue::CurPlayer->CurLeftSkill_ = CurSkillCode_;
